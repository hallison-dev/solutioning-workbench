...
const MODE_CONFIG = {
    ...
    logical: {
        ...
        targets: [
            ...
            { id: 'phase-pi-body', label: 'Payment Instruction', color: 'emerald' },
            { id: 'phase-rm-body', label: 'Remittance Data', color: 'amber' }
        ],
        nav: [
...
function renderStagingTable() {
    ...
    stagingData.forEach((rowObj, idx) => {
        ...
        dropdownWrapper.innerHTML = `
            <button class="action-dropdown-btn" onclick="openActionDropdown(this, event)">Assign <span>▾</span></button>
        `;
...
function openActionDropdown(btn, e) {
    ...
    MODE_CONFIG[currentMode].targets.forEach(t => {
        ...
        item.onclick = () => {
            assignStagingRow(rowIndex, t.id);
            menu.remove();
        };
...
// --- MAP MODAL LOGIC ---
let activeMapRowIndex = null;
function openMapModal(rowIndex) {
    activeMapRowIndex = rowIndex;
...
function removeMapping() {
    delete stagingData[activeMapRowIndex].mapping;
    saveState();
    renderStagingTable();
    closeMapModal();
}
...
function saveRowQuestion(rowIndex, qData) {
    if(!qData.text && !qData.answer) { delete stagingData[rowIndex].question; } 
    else { stagingData[rowIndex].question = qData; }
    saveState();
    renderStagingTable();
}
...
// Standard Boilerplate (Assign/Add/Edit/Unassign...)
function assignRow(btn, targetId) {}
function addRow(tbodyId) { createTableRow(document.getElementById(tbodyId), { spec: 'New Requirement', mapping: '', status: 'pending' }); saveState(); }
function editRow(btn) { const tr = btn.closest('tr'); const actionTd = tr.children[3]; actionTd.innerHTML = `<div class="flex gap-2 justify-end"><button class="text-xs text-blue-600" onclick="saveRowBtn(this)">Save</button><button class="text-xs text-red-600" onclick="saveRowBtn(this, true)">Unassign</button></div>`; }
function saveRowBtn(btn, unassign = false) {
    const tr = btn.closest('tr');
    if(unassign) {
        const specRaw = tr.children[0].innerText; 
        const m = specRaw.match(/Row\s+(\d+)[:\s]+(.*)/);
        if(m) { stagingData.push({ rowId: m[1], cells: [m[2]], category: '' }); } 
        else { stagingData.push({ rowId: '?', cells: [specRaw], category: '' }); }
        renderStagingTable(); tr.remove();
    } else {
...
function getAllSpecPool() { const set = new Set(); stagingData.forEach(row => { row.cells.forEach(c => set.add(c.trim())); }); MODE_CONFIG[currentMode].targets.map(t=>t.id).forEach(id => { const tb = document.getElementById(id); if(tb) Array.from(tb.querySelectorAll('tr')).forEach(tr=>set.add(tr.children[0].innerText.replace(tr.dataset.category||'','').trim())); }); return Array.from(set).filter(s=>s.length); }
...
function handleCSV(f) { const r = new FileReader(); r.onload = (e) => { const text = e.target.result; const lines = text.split(/\r?\n/).filter(l => l.trim() !== ''); const newRows = []; lines.forEach((line, idx) => { const cells = []; let cur = ''; let inQuotes = false; for (let i = 0; i < line.length; i++) { const ch = line[i]; if (ch === '"') { if (inQuotes && line[i+1] === '"') { cur += '"'; i++; } else inQuotes = !inQuotes; } else if (ch === ',' && !inQuotes) { cells.push(cur.trim().replace(/^"|"$/g, '')); cur = ''; } else { cur += ch; } } cells.push(cur.trim().replace(/^"|"$/g, '')); newRows.push({ rowId: idx+1, cells: cells }); }); stagingData = [...stagingData, ...newRows]; renderStagingTable(); saveState(); }; r.readAsText(f); }
